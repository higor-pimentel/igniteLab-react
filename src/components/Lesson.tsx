...
export function Lesson(props: LessonProps) {
  ...
  const availabreDateFormatted = format(props.availableAt, "EEEE' • 'd ' de 'MMMM' • 'k'h'mm", { locale: ptBr });

  const isActiveLesson = slug === props.slug;

  return (
    <Link to={`/event/lesson/${props.slug}`} className="group">
      <span className="text-gray-300">{availabreDateFormatted}</span>
      ...
      >
        <header className="flex items-center justify-between">
          ...
          <span
            className={classNames('text-xs rounded py-[0.123rem] px-2 text-white border font-bold', {
